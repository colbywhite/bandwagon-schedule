...
function getRankings(): Promise<number[]> {
  const source = "http://powerrankingsguru.com/nba/team-power-rankings.php";
  return axios
    .get(source)
...
function containsGamesWithinRange(
  ...
  const date = DateTime.fromFormat(gameDate, "M/d/yyyy h:mm:ss a", {
...
function getNBASchedule(
  ...
  return axios
    ...
    .then(({ leagueSchedule: { gameDates } }) =>
      gameDates.filter(containsValidGames).filter(isWithinWeek)
    );
}

function parseNBAStandings({
  resultSets,
}: RawNBAStandings): ParsedNBAStandings {
  ...
  return rawStandings.rowSet.reduce((standings, rawTeam) => {
    const parsedTeam: ParsedNBAStandingEntry = rawStandings.headers.reduce(
      (team, header, currentIndex) => {
...
function getNBAStandings(): Promise<ParsedNBAStandings> {
  ...
  return axios
    .get(STANDINGS_URL, { headers: HEADERS })
    .then(({ data }) => data as RawNBAStandings)
    .then(parseNBAStandings);
}
